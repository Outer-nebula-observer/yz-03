...
PAGES = [
 ...
 {"type":"content","tag":"04 · 实验设计与验证","title":"数据集与评估协议","layout":"numbered_list","items":["测试集：自建 50 条 8 类（A-H），查询与答案刻意避免关键词共现","对照组：固定用例集，通过依赖注入开关能力（G0-G6），不删减用例","指标：检索层 hit@3/5、MRR、NDCG；任务层 引用率/约束满足/噪声污染","统计：随机基线解析式、bootstrap CI、配对置换检验，种子固定 42"],"note":"【配图】无。\n【讲稿】强调‘对照是开关能力而不是删用例’。","image":""},
 ...
 {"type":"content","tag":"04 · 实验设计与验证","title":"跨场次与检索策略（RQ2/RQ3）","layout":"numbered_list","items":["跨场次：处理组 0.833 vs 对照 0，负控 0；机制路径完整，但 n=6 样本有限","p=0.059：方向性证据，需扩容到 20 条才能形成结论","检索：词袋下 hybrid 0.857 vs vector 0.929，召回未胜出","干扰压制：hybrid 1.0 高于任一单路 ≤0.875，排序更好","结论：混合价值在排序；真向量复测后再判定召回"],"note":"【配图】可放跨场次 WebUI 截图（第二场命中带‘往场沉淀’）或表格。\n【讲稿】对负结果要诚实。","image":"跨场次复用示意（WebUI 截图/表格）"},
